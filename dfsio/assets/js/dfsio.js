...
var pbuild = getUrlVars()["maprbuild"];
...
function drawRunInfoTable() {
  ...
  if (prunid || pbuild || pmfs || pdriver) {
    ...
    if (pbuild != null && pbuild != "") {
      if (param) { lurl += "&maprbuild=" + pbuild; }
      else { lurl += "?maprbuild=" + pbuild; }
      param = true;
    }
    if (pmfs!= null && pmfs!= "") {
    ...
    if (pdriver!= null && pdriver != "") {
      if (param) { lurl += "&mfsinstances=" + pdriver ; }
      else { lurl += "?mfsinstances=" + pdriver ;}
      param = true;
    }
    /* if (pdesc != null) {
...
function selectHandler() {
  ...
  for (var i=0; i<tabledata.getNumberOfColumns(); i++) {
    ...
    if (n.match(/Read/i) || n.match(/Build/i) || n.match(/Write/i)) {
      icols.push(i);
    }
  }
